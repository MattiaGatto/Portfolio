...
// PROCEDURE ASSEMBLY
extern void subMatAssembly64(MATRIX ds,int n,int k,MATRIX u_p,MATRIX v_p);
extern void euclidean_distance64(float* p, float* q, int k,float *res);
extern void dividiMatAssembly64(MATRIX vector, int nv, float costante, MATRIX res);
extern void prodMatrixAssembly64(MATRIX matrix1, MATRIX matrix2, int n1,int k1n2, int k2, MATRIX res);

/*
*  Metodi di supporto alla PCA
*
*/
...
void pca(params* input) {
    ...
    while(j<h){
      MATRIX temporaneo = alloc_matrix(1,1);
      prodMatrixAssembly64(u_p,u_p,1,n,1,temporaneo);
      float u_n= temporaneo[0];
      dealloc_matrix(temporaneo);
      temporaneo = alloc_matrix(k,1);
      prodMatrixAssembly64(ds_t,u_p,k,n,1,temporaneo);
      dividiMatAssembly64(temporaneo,k,u_n,v_p);  //Vettore dei load
      dealloc_matrix(temporaneo);
      temporaneo = alloc_matrix(1,1);
      prodMatrixAssembly64(v_p,v_p,1,k,1,temporaneo);
      float v_n= temporaneo[0];
      dividiMatAssembly64(v_p,k,sqrtf(v_n),v_p);
      float t = u_n;
      temporaneo = alloc_matrix(1,1);
      prodMatrixAssembly64(v_p,v_p,1,k,1,temporaneo);
      v_n = temporaneo[0];
      dealloc_matrix(temporaneo);
      temporaneo = alloc_matrix(n,1);
      prodMatrixAssembly64(ds,v_p,n,k,1,temporaneo);
      dividiMatAssembly64(temporaneo,n,v_n,u_p);
      dealloc_matrix(temporaneo);
      temporaneo = alloc_matrix(1,1);
      prodMatrixAssembly64(u_p,u_p,1,n,1,temporaneo);
      ...
      subMatAssembly64(ds,n,k,u_p,v_p);
      trasponi(ds,ds_t,n,k);
      j++;
    }
...
/*
* Metodi ausiliari del range query search
*
*/
...
float distance(MATRIX q, MATRIX acca, int k){
  ...
  float res=0;
  euclidean_distance64(p, q, k,&res);
  dealloc_matrix(p);
  return res;
}
...
int range_query_punto(KDTREE kdtree,MATRIX q_primo,int qid,float r,MATRIX acca,
  ...
  float res=0;
  euclidean_distance64(p,q_primo,k,&res);
  if( res<= r){
    qa[base*2] = qid;
...
void range_query(params* input) {
    ...
    MATRIX U = input->U;
    int n = input->n;
    int* qa = (int*) malloc(sizeof(int)*n*2*nq);
    ...
    if(h > 0){
      ...
      for(int i = 0; i < nq; i++){
        ...
        MATRIX q_primo = alloc_matrix(1,h);
        prodMatrixAssembly64(q,V,1,k,h,q_primo);
