...
extern void prodMatrixAssembly32(MATRIX matrix1, MATRIX matrix2, int n1,int k1n2, int k2, MATRIX res);

/*
*  Metodi ausiliari della PCA
*
*/
...
void pca(params* input) {
    ...
    while(j<h){
      ...
      prodMatrixAssembly32(ds,v_p,n,k,1,temporaneo);
...
void range_query(params* input) {
    ...
    MATRIX U = input->U;
    int n = input->n;
    int* qa = (int*) malloc(sizeof(int)*2*n*nq);
    if(qa == NULL){
      qa = (int*) malloc(sizeof(int)*n*nq);
    }
    ...
    if(h > 0){
      ...
      for(int i = 0; i < nq; i++){
        ...
        prodMatrixAssembly32(q,V,1,k,h,q_primo);
