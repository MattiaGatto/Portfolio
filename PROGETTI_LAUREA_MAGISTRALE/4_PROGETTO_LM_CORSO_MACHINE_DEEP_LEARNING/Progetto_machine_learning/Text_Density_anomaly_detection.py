...
def run_Density_Anomaly_T(path='testi-3.xlsx'):
    print("_____Start preprocessing_____\n")
    x_training, y_training, x_dig, id_anomaly=anomaly_data_text(path)
    ...
    f='2_testo/anomalydetection/anomaly_Text.hist'
    if os.path.isfile(f):
        history_dict = pickle.load(open(f, 'rb'))
        autoencoder.load_weights('2_testo/anomalydetection/anomaly_Text')
    else:
        history = autoencoder.fit(x_dig, x_dig, epochs=ep, batch_size=batches);
        history_dict = history.history;
        pickle.dump(history_dict, open(f, 'wb'))
        autoencoder.save_weights('2_testo/anomalydetection/anomaly_Text');

    print("\n_____outlierness_____\n")
    f='2_testo/anomalydetection/anomaly_out_Text.npy'
    if os.path.isfile(f):
